...
- 頁內嵌外站：一行 `@embed 網址`。框是一個小視窗：標題列可以拖、右下角可以拉大小、右上角「滿版」放到 96% 畫面（`Esc` 還原），標題列的「還原大小」回到原位。換頁或視窗大小變了會自動歸位。
- **寬度會改變對方站的長相**：響應式的站在窄框裡是手機版、寬框裡是桌機版。`@embed 網址 right 56% 70vh` 指定尺寸，或現場直接拉。
...
| `@embed 網址` | 右側嵌一個視窗（預設寬 32%，左欄字自動縮一級） |
| `@embed 網址 full` | 標題下方全寬視窗 |
| `@embed 網址 lazy` | 右側視窗，按「載入」才載 |
| `@embed 網址 right 56% 70vh` | 自己指定寬與高。順序不拘，`right`／`full`／`lazy` 以外的值就當尺寸 |
| `@img 網址 說明` | 圖片，點了放大 |
| `[文字](網址)` | 連結（開新分頁） |
| `` `code` `` | 行內程式碼 |
